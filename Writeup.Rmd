...
db <- db %>% dbGetQuery(joint.query) %>% collect() %>% 
  ...
  geom_curve(x = -73.9625, xend = -73.925, y = 40.725, yend = 40.7875, curvature = 0.01) + theme_fivethirtyeight() +theme(axis.title=element_text(size=12)) + ylab("Longitude") + xlab("Latitude") + ggtitle("Stations by Gender") + labs(subtitle="Plot depicts map of NYC") + scale_fill_discrete(name="Female to Male Ratio")
