...
This plot, which depicts a coordinate map of NYC, demonstrates that Citi Bike stations in more congested neighborhoods like downtown Manhattan are predominantly used by men, whereas stations in residential neighborhoods in the outer Boroughs see more female ridership, validating the claim that women tend to be wary of cycling in areas with less safety among car traffic. This gender distribution by station likely points to one of the main causes of the gender gap in urban cycling--safety?
...
For the second plot depicting gender distribution by trip duration,
...
For the last plot depicting gender distribution by station location, a dotplot mapping latitude and longitude was used to determine where in the city men and women were most often using the CitiBikes. Each dot represented a unique station, and they were color coded based on the ratio of female ridership to male ridership. Most of the values ranged between 0.0 and 1.2, meaning that in the majority of the stations more men were using the CitiBikes than women. Then, that dotplot was simplified to only show the ten stations with the lowest female-to-male ratio and the ten stations with the highest ratio. As it turns out, most of the stations with the lower ratios were located in Manhattan, while the ones with the higher ratios were all located in the outer boroughs (i.e. Brooklyn and Queens). 
